...
Webflow.define('webflow-forms', function ($) {
  ...
  function _submitForm(form, e) {
    ...
    // If there is an action specified, don't validate
    if ($.trim(form.attr('action')) !== '') {
      return;
    }
    ...
    form.find(':input:not([type="submit"])').each(function(i) {
      var fld = $(this), //  fld.data('name') ||
        name = fld.attr('name') || ('Field ' + (i + 1));
    ...
    var submitForm = $.trim(form.attr('data-submit-form')); //$('html').data('submit-form');

    if (submitForm) {
      var url = submitForm;

      // Work around same-protocol IE XDR limitation
      if (retro && url.indexOf('http://hybridmojo.com') >= 0) {
        url = url.replace('https://webflow.com/', 'http://hybridmojo.com/');
      }
      ...
      $.ajax({
        ...
        $(".w-form-done").show(); // .delay(10000).fadeOut();
        _afterSubmit(form, btn, wait);
      }).fail(function(xhr, status, err) {
        Recaptcha.reload();
        $(".w-form-fail").show(); //.delay(10000).fadeOut();
  ...
  function _afterSubmit(form, btn, wait, err) {
    ...
    if (!err) {
      ...
      // clear
            form.find(':input:not([type="submit"])').each(function(i) { i.value=""; });
